...
const UpcomingMeals = () => {
    ...
    useEffect(() => {
        // Recalculate total pages whenever filteredMeals changes
        const totalPages = Math.ceil(filteredMeals.length / 10 );
        setTotalPages(totalPages);
    }, [filteredMeals]);

    const paginatedFilteredMeals = filteredMeals?.slice(
    ...
    useEffect(() => {
        ...
        const slicedMeals = sortedMeals.slice(0, pageNumber * itemsPerPage);
                        ...
                        <div className="container duration-300">
                            <div className="w-full bg-white rounded-2xl overflow-hidden sm:shadow-lg my-5 duration-300">
                                ...
                                <div className="flex-1 sm:flex-none">
                                    {paginatedFilteredMeals?.map((meal) => (
                            ...
                            <div className="flex justify-center mt-4">
                                ...
                                {[...Array(totalPages).keys()].map((page) => (
                                    <button
                                        key={page + 1}
                                        className={`pb-1 m-1 border-2 rounded-full w-10 h-10 text-[18px] ${page + 1 === pageNumber ? 'bg-slate-400 text-white' : 'bg-white text-slate-400'
                                            }`}
